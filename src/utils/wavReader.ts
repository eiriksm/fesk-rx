...
export class WavReader {
  ...
  static async readWavFileInChunks(filePath: string, chunkSizeSeconds: number = 1.0): Promise<AudioSample[]> {
    const fullAudio = await this.readWavFile(filePath);
    const chunkSizeSamples = Math.floor(fullAudio.sampleRate * chunkSizeSeconds);
