...
export class SignalGenerator {
  ...
  generatePreambleSignal(sampleRate: number = 8000): AudioSample {
    const symbolDurationSamples = Math.floor(sampleRate * this.config.symbolDuration);
    const totalSamples = symbolDurationSamples * this.config.preambleBits.length;
  ...
  generateTernarySymbols(symbols: number[], sampleRate: number = 8000): AudioSample {
    const symbolDurationSamples = Math.floor(sampleRate * this.config.symbolDuration);
    ...
    for (let i = 0; i < symbols.length; i++) {
      ...
      switch (symbol) {
        case 0: frequency = f0; break;
        case 1: frequency = f1; break;
        case 2: frequency = f2; break;
        default: throw new Error(`Invalid symbol: ${symbol}`);
      }
