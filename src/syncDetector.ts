...
export class SyncDetector {
  ...
  private checkForSync(): SyncDetectionResult | null {
    ...
    for (const symbol of recentSymbols) {
      let bit: number;
      if (symbol.symbol === 0) { // f0
        bit = 0;
      } else if (symbol.symbol === 2) { // f2
        bit = 1;
      } else {
    ...
    const correlation = this.calculateCorrelation(detectedBits, this.config.barker13);
    ...
    if (correlation >= 11 && avgConfidence >= 0.5) { // Allow for 1-2 bit errors
      return {
        detected: true,
