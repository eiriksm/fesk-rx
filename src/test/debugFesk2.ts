...
async function debugFesk2() {
    ...
    try {
        ...
        try {
            ...
            console.log(`\\n🔢 Decoded to ${bytes.length} bytes: [${Array.from(bytes).map(b => '0x' + b.toString(16).padStart(2, '0')).join(', ')}]`);
            ...
            if (bytes.length >= 4) {
                ...
                console.log(`\\n📤 Header Analysis:`);
                console.log(`   Raw header: [0x${bytes[0].toString(16).padStart(2, '0')}, 0x${bytes[1].toString(16).padStart(2, '0')}]`);
                ...
                if (payloadLength > 0 && payloadLength <= 64 && bytes.length >= 2 + payloadLength + 2) {
                    ...
                    console.log(`\\n📦 Payload Analysis:`);
                    console.log(`   Raw payload: [${Array.from(bytes.slice(2, 2 + payloadLength)).map(b => '0x' + b.toString(16).padStart(2, '0')).join(', ')}]`);
                    ...
                    console.log(`   Need: ${2 + payloadLength + 2} bytes, have: ${bytes.length}`);
                }
            } else {
