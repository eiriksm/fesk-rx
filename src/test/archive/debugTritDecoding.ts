...
const expectedPayloadTrits = [2,2,0,0,2,1,2,0,0,1,0,1,2,1,1,2,0,1,0,0,0,1,1,0,1];

console.log('Expected scrambled bytes:', expectedScrambledBytes.map(b => '0x' + b.toString(16).padStart(2, '0')).join(', '));
...
function simulateTXPackingIncremental(bytes: number[]): number[] {
    ...
    for (let byteIdx = 0; byteIdx < bytes.length; byteIdx++) {
        ...
        packWork.push(byte);
        console.log(`  pack_work: [${packWork.map(b => '0x' + b.toString(16).padStart(2, '0')).join(', ')}]`);
        ...
        for (let r = remBatch.length - 1; r >= 0; r--) {
            ...
            console.log(`    pushed trit ${trit}, accumulator now has ${tritsInAccumulator} trits`);
        }
...
console.log(`Match: ${JSON.stringify(expectedPayloadTrits) === JSON.stringify(simulatedTrits) ? '✅' : '❌'}`);
