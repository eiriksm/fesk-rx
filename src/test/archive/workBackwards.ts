...
console.log(`Expected bytes: [${expectedBytes.map(b => '0x' + b.toString(16).padStart(2, '0')).join(', ')}]`);
...
console.log(`Header bytes: [${Array.from(headerBytes).map(b => '0x' + b.toString(16).padStart(2, '0')).join(', ')}]`);
...
if (payloadLength === 4) {
    ...
    console.log(`Payload bytes: [${Array.from(payloadBytes).map(b => '0x' + b.toString(16).padStart(2, '0')).join(', ')}]`);
    
    const payload = new Uint8Array(4);
    for (let i = 0; i < 4; i++) {
        payload[i] = descrambler.descrambleByte(payloadBytes[i]);
    }
    const text = Array.from(payload).map(b => String.fromCharCode(b)).join('');
    ...
    if (text === "test") {
        ...
        const testDecoded = knownScrambled.map(b => testDescrambler.descrambleByte(b));
        ...
        console.log(`\\nPayload comparison:`);
        console.log(`Expected scrambled: [${knownScrambled.map(b => '0x' + b.toString(16).padStart(2, '0')).join(', ')}]`);
        console.log(`Our decoded:        [${Array.from(payloadBytes).map(b => '0x' + b.toString(16).padStart(2, '0')).join(', ')}]`);
    }
} else {
