  ...
  try {
    ...
    for (const amp of amplificationLevels) {
      ...
      const maxAmp = Math.max(
        ...Array.from(amplifiedData.slice(0, testSamples)),
      );
      const minAmp = Math.min(
        ...Array.from(amplifiedData.slice(0, testSamples)),
      );
      console.log(
        `   Peak range: ${minAmp.toFixed(6)} to ${maxAmp.toFixed(6)}`,
      );
      ...
      for (
        let i = 0;
        i < Math.min(amplifiedData.length, audioData.sampleRate * 5);
        i += chunkSize
      ) {
        ...
        if (result) {
          frame = result;
          console.log(
            `   🎉 Frame decoded at ${audioSample.timestamp.toFixed(2)}s!`,
          );
          break;
        }
      }
      ...
      if (frame) {
        ...
        console.log(
          `   ✅ SUCCESS: "${message}" (CRC: ${frame.isValid ? "valid" : "invalid"})`,
        );
