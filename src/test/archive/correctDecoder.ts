...
console.log(
  `Preamble valid: ${JSON.stringify(expectedPreamble) === JSON.stringify(actualPreambleBits) ? "✅" : "❌"}`,
);
...
console.log(
  `Sync valid: ${JSON.stringify(expectedBarker13) === JSON.stringify(actualSyncBits) ? "✅" : "❌"}`,
);
...
console.log(
  `After pilot removal (${cleanedTrits.length} trits): [${cleanedTrits.join(",")}]`,
);
...
console.log(
  `\\n🔢 Decoded bytes (${allBytes.length}): [${Array.from(allBytes)
...
console.log(
  `\\n📤 Header: scrambled=[${Array.from(headerScrambled)
    .map((b) => "0x" + b.toString(16).padStart(2, "0"))
    .join(", ")}]`,
);
console.log(
  `          descrambled=[${Array.from(headerDescrambled)
    .map((b) => "0x" + b.toString(16).padStart(2, "0"))
    .join(", ")}]`,
);
...
if (
  ...
  console.log(
    `❌ Invalid payload length or insufficient data: need ${2 + payloadLength + 2} bytes, have ${allBytes.length}`,
  );
  process.exit(1);
}
...
console.log(
  `\\n📦 Payload: scrambled=[${Array.from(payloadScrambled)
    .map((b) => "0x" + b.toString(16).padStart(2, "0"))
    .join(", ")}]`,
);
console.log(
  `           descrambled=[${Array.from(payloadDescrambled)
    .map((b) => "0x" + b.toString(16).padStart(2, "0"))
    .join(", ")}]`,
);

const payloadText = Array.from(payloadDescrambled)
...
console.log(
  `\\n🔒 CRC: scrambled=[${Array.from(crcScrambled)
    .map((b) => "0x" + b.toString(16).padStart(2, "0"))
    .join(", ")}]`,
);
console.log(
  `       descrambled=[${Array.from(crcDescrambled)
    .map((b) => "0x" + b.toString(16).padStart(2, "0"))
    .join(", ")}]`,
);
console.log(
  `       received=0x${receivedCrc.toString(16).padStart(4, "0")}, calculated=0x${calculatedCrc.toString(16).padStart(4, "0")}`,
);
