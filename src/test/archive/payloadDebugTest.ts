...
async function payloadDebugTest() {
  ...
  try {
    ...
    for (
      let i = 0;
      i < Math.min(amplifiedData.length, audioData.sampleRate * 6);
      i += chunkSize
    ) {
      ...
      if (state.phase === "payload") {
        ...
        if (payloadChunksProcessed % 20 === 0) {
          console.log(
            `   Payload chunk ${payloadChunksProcessed}: tritBuffer=${state.tritBuffer.length}, tritCount=${state.tritCount}`,
          );
        ...
        if (
          state.tritBuffer.length >= 30 &&
          payloadChunksProcessed % 50 === 0
        ) {
          console.log(
          ...
          try {
            ...
            console.log(
              `   First 20 trits: [${tritBuffer.slice(0, 20).join(",")}]`,
            );
            console.log(
              `   Last 20 trits: [${tritBuffer.slice(-20).join(",")}]`,
            );
            ...
            console.log(
              `   Trit distribution: 0=${counts[0]}, 1=${counts[1]}, 2=${counts[2]}`,
            );
            ...
            if (total > 0) {
              ...
              console.log(
                `   Percentages: 0=${ratios[0]}%, 1=${ratios[1]}%, 2=${ratios[2]}%`,
              );
            }
          } catch (error) {
    ...
    console.log(`\\n📊 Summary:`);
    console.log(
      `   Preamble detected: ${preambleTime >= 0 ? preambleTime.toFixed(2) + "s" : "No"}`,
    );
    console.log(
      `   Sync detected: ${syncTime >= 0 ? syncTime.toFixed(2) + "s" : "No"}`,
    );
    console.log(`   Payload chunks processed: ${payloadChunksProcessed}`);
    console.log(`   Final phase: ${decoder.getState().phase}`);
    console.log(
      `   Final trit buffer size: ${decoder.getState().tritBuffer.length}`,
    );
  } catch (error) {
