...
async function singleChunkFesk2() {
  ...
  try {
    ...
    console.log(
      `   Duration: ${(audioData.data.length / audioData.sampleRate).toFixed(2)}s`,
    );
    console.log(`   Samples: ${audioData.data.length}`);

    // Check if signal level improved
    const rms = Math.sqrt(
      audioData.data.slice(0, 10000).reduce((sum, s) => sum + s * s, 0) / 10000,
    );
    console.log(
      `   RMS: ${rms.toFixed(6)} (${rms > 0.01 ? "Good" : rms > 0.001 ? "Moderate" : "Quiet"})`,
    );
    ...
    for (const { name, size } of chunkSizes) {
      ...
      for (let i = 0; i < amplifiedData.length; i += chunkSize) {
        ...
        if (result) {
          ...
          console.log(
            `   🎉 Frame ${frameCount}: "${message}" at ${audioSample.timestamp.toFixed(2)}s`,
          );
          console.log(
            `      Length: ${result.header.payloadLength}, CRC: ${result.isValid ? "✅" : "❌"}`,
          );
        ...
        if (name === "Single chunk") {
          ...
          console.log(
            `   Processed ${((endIndex / audioData.data.length) * 100).toFixed(1)}% - Phase: ${state.phase}, Trits: ${state.tritBuffer.length}`,
          );
      ...
      if (frameCount === 0) {
        ...
        console.log(
          `   ❌ No frames (Phase: ${finalState.phase}, Trits: ${finalState.tritBuffer.length})`,
        );
      } else {
