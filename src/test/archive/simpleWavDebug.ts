...
async function simpleWavDebug() {
  ...
  try {
    ...
    const estimatedFreq =
      (zeroCrossings / 2) * (audioData.sampleRate / analysisSamples);
    console.log(
      `   Estimated dominant frequency: ${estimatedFreq.toFixed(1)} Hz`,
    );

    // Check if it's in our expected range
    const expectedRange = [2400, 3600];
    if (
      estimatedFreq >= expectedRange[0] &&
      estimatedFreq <= expectedRange[1]
    ) {
      console.log(
        `   ✅ Frequency is in FESK range (${expectedRange[0]}-${expectedRange[1]} Hz)`,
      );
    } else {
      console.log(
        `   ⚠️  Frequency outside FESK range (expected ${expectedRange[0]}-${expectedRange[1]} Hz)`,
      );
    }
  } catch (error) {
