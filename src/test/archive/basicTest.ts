...
console.log(`Header descrambled: hi=0x${hiDescrambled.toString(16).padStart(2, '0')}, lo=0x${loDescrambled.toString(16).padStart(2, '0')}`);
console.log(`Payload length: ${payloadLength}`);

// Test 3: TritDecoder with payload trits
console.log('\\n3. Testing TritDecoder with payload trits');
const payloadTrits = [2,2,0,0,2,1,2,0,0,1,0,1,2,1,1,2,0,1,0,0,0,1,1,0,1]; // From the TX output
...
console.log(`Descrambled payload: [${Array.from(descrambledPayload).map(b => '0x' + b.toString(16).padStart(2, '0')).join(', ')}]`);
console.log(`As text: "${Array.from(descrambledPayload).map(b => String.fromCharCode(b)).join('')}"`);
