...
const preambleMatch = JSON.stringify(preambleExpected) === JSON.stringify(preambleActual);
console.log(`Preamble (0-11): [${preambleActual.join(',')}] ${preambleMatch ? '✅' : '❌'}`);
...
const barker13Match = JSON.stringify(barker13Expected) === JSON.stringify(barker13Actual);
console.log(`Sync (12-24): [${barker13Actual.join(',')}] ${barker13Match ? '✅' : '❌'}`);
...
if (preambleMatch && barker13Match) {
    ...
    if (payloadLength > 0 && payloadLength <= 64) {
        ...
        const payloadEnd = Math.min(sequence.length - 9, payloadStart + expectedPayloadTrits); // Leave 9 for CRC
        ...
        if (crcTrits.length >= 8) {
            ...
            for (const trit of crcTrits.slice(0, 9)) { // Take up to 9 trits for CRC
