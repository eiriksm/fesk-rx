...
async function completeFesk2() {
    ...
    try {
        ...
        console.log(`📄 Processing ${(audioData.data.length / audioData.sampleRate).toFixed(1)}s of audio...`);
        ...
        for (let i = 0; i < amplifiedData.length; i += chunkSize) {
            ...
            if (state.phase === 'payload') {
                ...
                if (chunkNum % 30 === 0) { // Every 3 seconds
                    const elapsed = audioSample.timestamp - payloadStartTime;
                    console.log(`  Payload: ${state.tritBuffer.length} trits (${elapsed.toFixed(1)}s in payload)`);
                }
            }
            ...
            if (result) {
                ...
                console.log(`   CRC: 0x${result.crc.toString(16).padStart(4, '0')} ${result.isValid ? '✅' : '❌'}`);
            ...
            if (state.phase === 'payload' && payloadStartTime > 0) {
                ...
                if (timeInPayload > 8.0) { // 8 seconds is very long for a payload
                    ...
                    try {
                        ...
                        if (bytes.length >= 2) {
                            ...
                            if (payloadLength > 0 && payloadLength <= 50) {
                                console.log(`   Length seems reasonable - might need more trits`);
                            }
                        }
                    } catch (error) {
        ...
        console.log(`   File duration: ${(audioData.data.length / audioData.sampleRate).toFixed(1)}s`);
        
        if (frameCount === 0) {
            console.log(`\\n❌ No complete frames decoded`);
            console.log(`   This might indicate the transmission is longer than expected`);
