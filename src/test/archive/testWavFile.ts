  ...
  try {
    ...
    for (let i = 0; i < audioData.data.length; i += chunkSize) {
      ...
      if (chunkCount % 100 === 0) {
        ...
        console.log(
          `   Chunk ${chunkCount}: phase=${state.phase}, tritBuffer=${state.tritBuffer.length}`,
        );
      }
    }
    ...
    if (frame) {
      ...
      console.log("==========================");
      console.log(
        `Header: payload length = ${frame.header.payloadLength} bytes`,
      );
      console.log(
        `Payload bytes: [${Array.from(frame.payload)
      ...
      if (finalState.tritBuffer.length > 0) {
        console.log(
          `   Last 20 trits: [${finalState.tritBuffer.slice(-20).join(",")}]`,
        );
      }
    }
  } catch (error) {
