...
console.log(
  `Payload: "${payload.toString()}" = [${Array.from(payload)
...
console.log(
  `Header: length=${payloadLength} -> hi=0x${headerHi.toString(16).padStart(2, "0")}, lo=0x${headerLo.toString(16).padStart(2, "0")}`,
);
...
console.log(
  `Scrambled header: hi=0x${scrambledHeaderHi.toString(16).padStart(2, "0")}, lo=0x${scrambledHeaderLo.toString(16).padStart(2, "0")}`,
);

const scrambledPayload = Array.from(payload).map((byte) =>
  lfsr.scrambleByte(byte),
);
console.log(
  `Scrambled payload: [${scrambledPayload.map((b) => "0x" + b.toString(16).padStart(2, "0")).join(", ")}]`,
);
...
console.log(
  `CRC bytes: hi=0x${crcHi.toString(16).padStart(2, "0")}, lo=0x${crcLo.toString(16).padStart(2, "0")}`,
);
...
const actualPayloadTrits = [
  2, 2, 0, 0, 2, 1, 2, 0, 0, 1, 0, 1, 2, 1, 1, 2, 0, 1, 0, 0, 0, 1, 1, 0, 1,
]; // From positions 31-55
const actualCrcTrits = [0, 1, 1, 2, 1, 1, 1, 2, 1]; // From positions 56-64

console.log(`\\n🔍 Comparison with actual transmission:`);
console.log(
  `Header - Expected: [${headerTrits.join(",")}], Actual: [${actualHeaderTrits.join(",")}], Match: ${JSON.stringify(headerTrits) === JSON.stringify(actualHeaderTrits) ? "✅" : "❌"}`,
);
console.log(
  `Payload - Expected: [${payloadTrits.join(",")}], Actual: [${actualPayloadTrits.join(",")}], Match: ${JSON.stringify(payloadTrits) === JSON.stringify(actualPayloadTrits) ? "✅" : "❌"}`,
);
console.log(
  `CRC - Expected: [${crcTrits.join(",")}], Actual: [${actualCrcTrits.join(",")}], Match: ${JSON.stringify(crcTrits) === JSON.stringify(actualCrcTrits) ? "✅" : "❌"}`,
);
