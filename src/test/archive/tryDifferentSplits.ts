...
for (const headerLen of headerLengths) {
    ...
    console.log(`Descrambled: hi=0x${headerHi.toString(16)}, lo=0x${headerLo.toString(16)} -> length=${payloadLength}`);
    ...
    if (payloadLength > 0 && payloadLength <= 32) {
        ...
        const payloadTrits = remainingTrits.slice(0, remainingTrits.length - estimatedCrcTrits);
        
        console.log(`Payload trits (${payloadTrits.length}): [${payloadTrits.join(',')}]`);
        ...
        console.log(`Scrambled: [${Array.from(scrambledPayload).map(b => '0x' + b.toString(16).padStart(2, '0')).join(', ')}]`);
        ...
        if (crcTrits.length >= 8) {
            ...
            console.log(`CRC: received=0x${receivedCrc.toString(16).padStart(4, '0')}, calculated=0x${calculatedCrc.toString(16).padStart(4, '0')}, match=${receivedCrc === calculatedCrc ? '✅' : '❌'}`);
        }
    }
}
