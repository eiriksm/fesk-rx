...
async function workingWavTest() {
    ...
    try {
        ...
        for (let i = 0; i < Math.min(amplifiedData.length, audioData.sampleRate * 8); i += chunkSize) {
            ...
            if (state.phase === 'payload') {
                ...
                if (chunkNum % 10 === 0) {
                    console.log(`     Chunk ${chunkNum}: ${state.tritBuffer.length} trits collected`);
                }
            }
        ...
        console.log(`Final state: phase=${finalState.phase}, tritBuffer=${finalState.tritBuffer.length}`);
        ...
        if (finalState.phase === 'payload' && finalState.tritBuffer.length > 0) {
            ...
            console.log(`   First 20: [${finalState.tritBuffer.slice(0, 20).join(',')}]`);
            console.log(`   Last 20: [${finalState.tritBuffer.slice(-20).join(',')}]`);
            ...
            try {
                ...
                if (bytes.length >= 4) {
                    ...
                    if (payloadLength > 0 && payloadLength <= 20) {
                        console.log(`   Payload length looks reasonable, issue might be elsewhere`);
                    }
                }
            } catch (error) {
