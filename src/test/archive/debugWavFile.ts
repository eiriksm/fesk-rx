...
async function debugWavFile() {
    ...
    try {
        ...
        console.log(`   Peak amplitude: ${Math.max(...Array.from(audioData.data)).toFixed(4)}`);
        console.log(`   Min amplitude: ${Math.min(...Array.from(audioData.data)).toFixed(4)}`);
        ...
        for (let i = 0; i < Math.min(audioData.data.length, audioData.sampleRate * 2); i += chunkSize) {
            ...
            if (detections.length > 0 && totalToneDetections < 20) {
                console.log(`   Chunk ${Math.floor(i / chunkSize)} (${audioSample.timestamp.toFixed(2)}s): ${detections.length} detections`);
                detections.forEach((det, idx) => {
                    console.log(`     ${idx}: freq=${det.frequency.toFixed(1)}Hz, mag=${det.magnitude.toFixed(4)}, conf=${det.confidence.toFixed(3)}`);
                });
            }
        }
        ...
        console.log(`   Expected frequencies: f0=${config.toneFrequencies[0]}Hz, f1=${config.toneFrequencies[1]}Hz, f2=${config.toneFrequencies[2]}Hz`);
        
        if (totalToneDetections === 0) {
            console.log('\\n🔍 No tone detections - checking signal properties...');
            
            // Analyze first second of audio
            const analysisLength = Math.min(audioData.sampleRate, audioData.data.length);
            ...
            if (energy < 0.001) {
                console.log('   ⚠️  Signal appears very quiet - might need gain adjustment');
            }
            
            // Basic frequency content check using a simple approach
            console.log('   Checking for energy around expected frequencies...');
            const testChunk = audioData.data.slice(0, Math.min(8192, audioData.data.length));
            ...
            try {
                ...
                console.log(`   Forced detection attempt: ${debugDetections.length} results`);
                if (debugDetections.length > 0) {
                    debugDetections.slice(0, 5).forEach((det, idx) => {
                        console.log(`     ${idx}: ${det.frequency.toFixed(1)}Hz (conf: ${det.confidence.toFixed(3)})`);
                    });
                }
            } catch (error) {
