...
const toneSequence = [
  ...
  1, 0, 2, 1, 1, 1, 0, 0, 2, 1, 0, 0, 1, 0, 2, 1, 2, 2, 2, 0, 2, 0, 2, 1, 1, 2,
  1, 1, 0, 2, 1, 2, 2, 0, 2, 0, 0, 2, 1, 1, 2, 2, 2, 1, 1, 2, 1, 2, 2, 0, 0,
];
...
const actualPreambleBits = toneSequence
  .slice(0, 12)
  .map((s) => (s === 2 ? 1 : 0));
console.log(
...
console.log(
  `Sync check: ${JSON.stringify(expectedBarker13) === JSON.stringify(actualSyncBits) ? "✅" : "❌"}`,
);
...
console.log(
  `\\n🔢 Decoded bytes: [${Array.from(allBytes)
...
console.log(
  `\\n📤 Header: [${Array.from(headerScrambled)
    .map((b) => "0x" + b.toString(16).padStart(2, "0"))
    .join(", ")}] -> length=${payloadLength}`,
);
...
if (
  ...
  console.log(
    `❌ Invalid setup: payloadLength=${payloadLength}, totalBytes=${allBytes.length}`,
  );
  process.exit(1);
}
...
console.log(
  `\\n📦 Payload: [${Array.from(payloadScrambled)
    .map((b) => "0x" + b.toString(16).padStart(2, "0"))
    .join(", ")}] -> "${payloadText}"`,
);
