...
function lfsrDebug() {
  ...
  console.log(
    `   Initial LFSR state: 0x${descrambler["state"].toString(16).padStart(3, "0")}`,
  );
  ...
  for (let i = 0; i < 8; i++) {
    ...
    console.log(
      `     Bit ${i}: LFSR=0x${state.toString(16).padStart(3, "0")} lsb=${lfsrBit}, input=${inputBit}, output=${outputBit}`,
    );
    ...
    console.log(
      `       Next LFSR state: 0x${state.toString(16).padStart(3, "0")} (feedback=${feedback})`,
    );
  }
  ...
  console.log(
    `   Class result: 0x${classResult.toString(16).padStart(2, "0")}`,
  );
