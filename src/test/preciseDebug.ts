...
function preciseDebug() {
    ...
    for (let tritCount = 125; tritCount <= 135; tritCount++) {
        ...
        console.log(`   ${tritCount} trits → ${byteCount} bytes ${byteCount === 26 ? '🎯' : ''}`);
        
        if (byteCount === 26) {
            console.log(`   ✅ FOUND IT! Need exactly ${tritCount} trits`);
            console.log(`   Must remove ${payload.length - tritCount} trits from payload`);
            break;
        }
    }
    ...
    for (const pos of pilotCandidates) {
        if (pos < payload.length - 1 && payload[pos] === 0 && payload[pos + 1] === 2) {
            console.log(`   Pilot candidate at position ${pos}: [${payload[pos]}, ${payload[pos + 1]}]`);
            ...
            const byteCount = decoder.getBytes().length;
            console.log(`     Removing pilot at ${pos}: ${payload.length} → ${testTrits.length} trits → ${byteCount} bytes`);
        } else {
            console.log(`   No pilot at expected position ${pos}`);
            if (pos < payload.length) {
                console.log(`     Found: [${payload[pos]}, ${pos + 1 < payload.length ? payload[pos + 1] : 'END'}]`);
            }
        }
    }
    ...
    if (payload[64] === 0 && payload[65] === 2) {
        ...
        if (adjustedPos < testTrits.length - 1 && testTrits[adjustedPos] === 0 && testTrits[adjustedPos + 1] === 2) {
            testTrits.splice(adjustedPos, 2);
            removedCount += 2;
            console.log(`   Removed pilot at adjusted position ${adjustedPos} (originally ~128)`);
        }
    }
    ...
    console.log(`\n📊 Final result:`);
    console.log(`   ${payload.length} → ${testTrits.length} trits (removed ${removedCount}))`);
