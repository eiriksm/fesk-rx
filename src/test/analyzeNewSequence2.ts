...
async function analyzeNewSequence2() {
    ...
    for (let i = 0; i < payload.length - 1; i++) {
        if (payload[i] === 0 && payload[i + 1] === 2) {
            console.log(`   Found [0,2] at positions ${i},${i+1} (absolute: ${25+i},${25+i+1})`);
        }
    }
...
async function tryDecode(trits: number[], indent: string = ''): Promise<boolean> {
    try {
        ...
        console.log(`${indent}Header: [0x${bytes[0].toString(16).padStart(2,'0')}, 0x${bytes[1].toString(16).padStart(2,'0')}] → [0x${headerHi.toString(16).padStart(2,'0')}, 0x${headerLo.toString(16).padStart(2,'0')}] → ${payloadLength} bytes`);
        
        if (payloadLength <= 0 || payloadLength > 64 || bytes.length < 2 + payloadLength + 2) {
            console.log(`${indent}❌ Invalid payload length or insufficient data (need ${2 + payloadLength + 2}, have ${bytes.length})`);
            return false;
        }
