...
async function finalTestSequence2() {
    ...
    console.log(`   ${payload.length} → ${cleanedTrits.length} trits (removed ${payload.length - cleanedTrits.length} pilots)`);
    ...
    while (i < trits.length) {
        ...
        if (dataCount > 0 && dataCount % PILOT_INTERVAL === 0) {
            ...
            if (i < trits.length - 1 && trits[i] === 0 && trits[i + 1] === 2) {
                console.log(`   Found and removed pilot [0,2] at data position ${dataCount}`);
...
async function tryDecode(trits: number[]): Promise<boolean> {
    ...
    try {
        ...
        console.log(`   Header: 0x${headerHi.toString(16).padStart(2,'0')}${headerLo.toString(16).padStart(2,'0')} → ${payloadLength} bytes`);
        ...
        if (payloadLength <= 0 || payloadLength > 64) {
            ...
            console.log(`   Either the transmission was incomplete, corrupted, or not generated properly.`);
            return false;
        }
        ...
        console.log(`   CRC: 0x${receivedCrc.toString(16).padStart(4,'0')} ${isValid ? '✅' : '❌'}`);
