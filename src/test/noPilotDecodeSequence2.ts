...
async function tryDecode(trits: number[], indent: string = ''): Promise<boolean> {
    try {
        ...
        if (bytes.length < 4) {
            console.log(`${indent}${trits.length} trits → ${bytes.length} bytes (insufficient)`);
            return false;
        }
        ...
        console.log(`${indent}${trits.length} trits → ${bytes.length} bytes → header: ${payloadLength} payload bytes`);
        ...
        if (payloadLength > 0 && payloadLength <= 64 && bytes.length >= 2 + payloadLength + 2) {
            ...
            console.log(`${indent}   CRC: ${receivedCrc === calculatedCrc ? '✅' : '❌'} (0x${receivedCrc.toString(16).padStart(4, '0')})`);
