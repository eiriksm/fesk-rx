...
async function flexibleDecodeSequence2() {
  ...
  console.log(
    `   ${payload.length} → ${aggressive.length} trits (removed ${payload.length - aggressive.length})`,
  );
  ...
  console.log(
    `   ${payload.length} → ${selective.length} trits (removed ${payload.length - selective.length})`,
  );
  ...
  console.log(
    `   ${payload.length} → ${expected.length} trits (removed ${payload.length - expected.length})`,
  );
...
function removeSelectivePilots(trits: number[]): number[] {
  ...
  while (i < trits.length) {
    ...
    if (
      ...
      console.log(
        `   Removing [0,2] at data position ${dataCount} (boundary check)`,
      );
      i += 2; // Skip both
    } else {
      result.push(trits[i]);
...
function removeExpectedPilots(trits: number[]): number[] {
  ...
  while (i < trits.length) {
    if (
      pilotPositions.includes(dataCount) &&
      i < trits.length - 1 &&
      trits[i] === 0 &&
      trits[i + 1] === 2
    ) {
...
async function tryDecode(
  ...
  try {
    ...
    console.log(
      `${indent}Header: 0x${headerHi.toString(16).padStart(2, "0")} 0x${headerLo.toString(16).padStart(2, "0")} → ${payloadLength} bytes`,
    );
    ...
    if (receivedCrc === calculatedCrc) {
      console.log(
        `${indent}🎉 SUCCESS: "${message}" (CRC: 0x${receivedCrc.toString(16).padStart(4, "0")})`,
      );
      return true;
    } else {
