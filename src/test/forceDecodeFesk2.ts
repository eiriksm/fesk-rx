...
async function forceDecodeFesk2() {
    ...
    try {
        ...
        for (let i = 0; i < amplifiedData.length; i += chunkSize) {
            ...
            if (state.phase === 'payload' && state.tritBuffer.length > maxTrits.length) {
...
async function tryDecode(trits: number[], indent: string = ''): Promise<boolean> {
    try {
        ...
        console.log(`${indent}Decoded to ${bytes.length} bytes: [${Array.from(bytes).map(b => '0x' + b.toString(16).padStart(2, '0')).join(', ')}]`);
