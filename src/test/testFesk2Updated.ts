...
async function testFesk2Updated() {
    ...
    try {
        ...
        console.log(`   Signal quality: ${rms > 0.001 ? '✅ Good' : '⚠️  Still quiet'}`);

        // Determine amplification needed
        let amplification = 1;
        if (rms < 0.001) {
            amplification = 1000;
            console.log(`   Applying ${amplification}x amplification (signal still quiet)`);
        } else if (rms < 0.01) {
            amplification = 10;
            console.log(`   Applying ${amplification}x amplification (signal moderate)`);
        } else {
        ...
        console.log(`   File processed: ${(amplifiedData.length / audioData.sampleRate).toFixed(2)}s`);
        
        if (frameCount === 0) {
            console.log('\\n❌ No frames decoded');
            const finalState = decoder.getState();
            console.log(`   Final phase: ${finalState.phase}`);
            console.log(`   Final trit buffer: ${finalState.tritBuffer.length} trits`);
        } else {
