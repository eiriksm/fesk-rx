...
async function decodeNewSequence2() {
  ...
  console.log(
    `   Payload trits (first 50): [${payload.slice(0, 50).join(",")}...]`,
  );
  ...
  try {
    ...
    console.log(
      `   Cleaned: ${cleanedTrits.length} trits (removed ${payload.length - cleanedTrits.length} pilots)`,
    );
    ...
    console.log(
      `   Decoded to ${allBytes.length} bytes: [${Array.from(allBytes)
    ...
    console.log(`\n🔓 Header descrambling:`);
    console.log(
      `   Raw header bytes: [0x${allBytes[0].toString(16).padStart(2, "0")}, 0x${allBytes[1].toString(16).padStart(2, "0")}]`,
    );
    console.log(
    ...
    if (allBytes.length < 2 + payloadLength + 2) {
      console.log(
        `❌ Need ${2 + payloadLength + 2} bytes total, have ${allBytes.length}`,
      );
      return;
    }
    ...
    console.log(`\n📦 Payload processing:`);
    console.log(
      `   Raw payload: [${Array.from(allBytes.slice(2, 2 + payloadLength))
        .map((b) => "0x" + b.toString(16).padStart(2, "0"))
        .join(", ")}]`,
    );
    console.log(
      `   Descrambled: [${Array.from(payloadBytes)
        .map((b) => "0x" + b.toString(16).padStart(2, "0"))
        .join(", ")}]`,
    );

    // Decode message
    const message = Array.from(payloadBytes)
      .map((b) => {
