...
async function testProperPilotRemoval() {
  ...
  console.log(
    `📊 After pilot removal: ${cleanedTrits.length} trits (removed ${payload.length - cleanedTrits.length})`,
  );
  ...
  while (i < trits.length) {
    ...
    if (dataCount > 0 && dataCount % PILOT_INTERVAL === 0) {
      ...
      if (i < trits.length - 1 && trits[i] === 0 && trits[i + 1] === 2) {
        ...
        console.log(
          `   No pilot found at expected position ${i}, continuing (tolerance)`,
        );
...
async function tryDecode(trits: number[]): Promise<boolean> {
  ...
  try {
    ...
    console.log(`\n🔓 Header analysis:`);
    console.log(
      `   Raw: [0x${bytes[0].toString(16).padStart(2, "0")}, 0x${bytes[1].toString(16).padStart(2, "0")}]`,
    );
    console.log(
    ...
    console.log(`\n📦 Payload:`);
    console.log(
      `   Raw: [${Array.from(bytes.slice(2, 2 + payloadLength))
        .map((b) => "0x" + b.toString(16).padStart(2, "0"))
        .join(", ")}]`,
    );
    console.log(
