...
async function tryDecodeSubset(
  trits: number[],
  indent: string = "",
  log: boolean = true,
): Promise<any> {
  try {
    ...
    if (log)
      console.log(`${indent}Trits: ${trits.length} → Bytes: ${bytes.length}`);
    ...
    if (log) {
      ...
      console.log(
        `${indent}CRC: ${isValid ? "✅" : "❌"} (0x${receivedCrc.toString(16).padStart(4, "0")})`,
      );
    }
