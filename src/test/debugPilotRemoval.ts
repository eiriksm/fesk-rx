...
function debugPilotRemoval() {
    ...
    console.log(`   Strategy 1: ${bytes1 === 26 ? '✅' : '❌'} (${bytes1} bytes)`);
    console.log(`   Strategy 2: ${bytes2 === 26 ? '✅' : '❌'} (${bytes2} bytes)`);
    console.log(`   Strategy 3: ${bytes3 === 26 ? '✅' : '❌'} (${bytes3} bytes)`);
}
...
function removeSmartPilots(trits: number[]): number[] {
    ...
    while (i < trits.length && cleaned.length < expectedDataTrits) {
        ...
        if (i < trits.length - 1 && trits[i] === 0 && trits[i + 1] === 2) {
            ...
            if (wouldEndUp > expectedDataTrits * 1.1) { // If we'd overshoot, it's probably a pilot
