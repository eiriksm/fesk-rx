        ...
        if (this.tritCount >= 8) {
          ...
          if (headerBytes[0] !== 0 || headerBytes[1] !== 0) {
            ...
            if (this.payloadLength > 0 && this.payloadLength <= 256) {
              console.log(
                `Header decoded: payload length = ${this.payloadLength} bytes`,
              );
  ...
  private handlePayloadPhase(
    ...
    for (const detection of toneDetections) {
      ...
      if (symbol !== null) {
        ...
        if (this.tritCount % 10 === 0) {
          // Check every 10 trits
          const availableBytes = this.payloadDecoder.extractExactBytes(
            this.payloadLength,
          );
          ...
          if (nonZeroBytes >= this.payloadLength) {
            ...
            for (let i = 0; i < this.payloadLength; i++) {
              descrambledPayload[i] = this.descrambler.descrambleByte(
                availableBytes[i],
              );
            }

            console.log(
              `Payload decoded: "${Array.from(descrambledPayload)
