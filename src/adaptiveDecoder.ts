...
export class AdaptiveFeskDecoder {
  ...
  private searchForPreamble(audioSample: AudioSample): Frame | null {
    ...
    if (result) {
      ...
      console.log('  Measured symbol duration:', result.symbolDuration.toFixed(3) + 's (' + (result.symbolDuration * 1000).toFixed(1) + 'ms)');
      console.log('  Timing ratio vs spec:', (result.symbolDuration / this.config.symbolDuration).toFixed(3) + 'x');
  ...
  private detectPreambleWithAdaptiveTiming(audioSample: AudioSample): {startTime: number, symbolDuration: number} | null {
    ...
    let bestResult: {startTime: number, symbolDuration: number, accuracy: number} | null = null;
    
    for (let startSample = 0; startSample < audioSample.data.length - windowSize; startSample += hopSize) {
      const windowData = audioSample.data.slice(startSample, startSample + windowSize);
      const windowStartTime = audioSample.timestamp / 1000 + startSample / audioSample.sampleRate;
      
      const windowAudio = {
        data: windowData,
        sampleRate: audioSample.sampleRate,
        timestamp: audioSample.timestamp + (startSample / audioSample.sampleRate) * 1000
      };
      
      // Test each duration
      for (const testDuration of testDurations) {
        const result = this.testPreambleAtTiming(windowAudio, windowStartTime, testDuration);
        
        if (result && result.accuracy > 0.8 && (!bestResult || result.accuracy > bestResult.accuracy)) {
          bestResult = result;
        }
      }
    }
  ...
  private testPreambleAtTiming(audioSample: AudioSample, startTime: number, symbolDuration: number): {startTime: number, symbolDuration: number, accuracy: number} | null {
    ...
    for (let i = 0; i < expectedPreamble.length; i++) {
      ...
      const sampleEnd = Math.min(sampleStart + symbolSamples, audioSample.data.length);
      ...
      for (const detection of detections) {
        ...
        if (symbol >= 0 && detection.confidence > bestConfidence && detection.confidence > 0.7) {
          bestSymbol = symbol;
  ...
  private processSync(audioSample: AudioSample): Frame | null {
    ...
    const syncLength = 13;
    const syncStartTime = this.transmissionStartTime + (preambleLength * this.measuredSymbolDuration);
    
    const syncResult = this.extractSequence(audioSample, syncStartTime, syncLength, this.measuredSymbolDuration);
    ...
    if (syncResult) {
      ...
      if (accuracy >= 0.7) {
        console.log('✅ Sync detected with', (accuracy * 100).toFixed(1) + '% accuracy');
  ...
  private processHeader(audioSample: AudioSample): Frame | null {
    ...
    const headerStartTime = this.transmissionStartTime + (headerStartSymbols * this.measuredSymbolDuration);
    
    const headerResult = this.extractSequence(audioSample, headerStartTime, headerLength, this.measuredSymbolDuration);
  ...
  private extractSequence(audioSample: AudioSample, startTime: number, length: number, symbolDuration: number): number[] | null {
    const symbols: number[] = [];
    const symbolSamples = Math.floor(audioSample.sampleRate * symbolDuration);
    ...
    for (let i = 0; i < length; i++) {
      ...
      const sampleOffset = Math.floor((symbolTime - audioStartTime) * audioSample.sampleRate);
      
      if (sampleOffset < 0 || sampleOffset + symbolSamples >= audioSample.data.length) {
        continue; // Symbol not in this audio chunk
      }
      
      const symbolData = audioSample.data.slice(sampleOffset, sampleOffset + symbolSamples);
