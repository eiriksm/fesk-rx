...
export class FeskPayloadDecoder {
  ...
  static decodePayload(
    ...
    try {
      ...
      console.log(
        `Scrambled bytes: [${Array.from(scrambledBytes)
      ...
      console.log(
        `Descrambled: [${Array.from(descrambledBytes)
          .map((b) => "0x" + b.toString(16).padStart(2, "0"))
          .join(", ")}]`,
      );
      console.log(
        `As text: "${Array.from(descrambledBytes)
      ...
      console.log(
        `CRC received: 0x${receivedCRC.toString(16).padStart(4, "0")}`,
      );
      console.log(
        `CRC calculated: 0x${calculatedCRC.toString(16).padStart(4, "0")}`,
      );
    ...
    for (let i = 0; i < trits.length; i++) {
      if (dataTrits > 0 && dataTrits % 64 === 0) {
        if (i + 1 < trits.length && trits[i] === 0 && trits[i + 1] === 2) {
          console.log(
            `Found pilot [${trits[i]},${trits[i + 1]}] at position ${dataTrits}`,
          );
  ...
  private static extractCRC(crcTrits: number[]): number {
    ...
    if (crcBytes.length < 2) {
      throw new Error(
        `Insufficient CRC data: got ${crcBytes.length} bytes, expected 2`,
      );
    }
